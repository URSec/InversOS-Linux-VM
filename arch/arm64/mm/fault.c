					   ...
					   unsigned long addr)
{
	...
	/*
	 * Filter out instruction/data aborts generated by inversos tasks as
	 * they run in the same exception level as the kernel.
	 */
	if (task_inversos(current) && user_mode(regs))
		return false;
#endif
...
static void __do_user_fault(struct siginfo *info, unsigned int esr)
{
	...
	if (current->thread.fault_address >= TASK_SIZE) {
		...
		case ESR_ELx_EC_DABT_CUR:
			/* user_mode(regs) checked before reaching here */
			if (!task_inversos(current)) {
				WARN(1, "ESR 0x%x is not DABT from EL0\n", esr);
				esr = 0;
				break;
			}
			/* Fall-through */
		...
		case ESR_ELx_EC_IABT_CUR:
			/* user_mode(regs) checked before reaching here */
			if (!task_inversos(current)) {
				WARN(1, "ESR 0x%x is not IABT from EL0\n", esr);
				esr = 0;
				break;
			}
			/* Fall-through */
...
#ifdef CONFIG_ARM64_INVERSOS
static bool is_el0_inversos_instruction_abort(unsigned int esr,
					      struct pt_regs *regs)
{
	return task_inversos(current) && user_mode(regs) &&
